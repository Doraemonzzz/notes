# adapt from https://github.com/Lightning-AI/torchmetrics/blob/master/src/torchmetrics/image/fid.py, use precomputed feature
...
def _compute_fid(mu1: Tensor, sigma1: Tensor, mu2: Tensor, sigma2: Tensor) -> Tensor:
    ...
    """
    print(torch.mean(mu1), torch.mean(sigma1), torch.mean(mu2), torch.mean(sigma2))
...
class FrechetInceptionDistance(Metric):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        fid_statistics_file = kwargs.pop("fid_statistics_file", None)
        ...
        if isinstance(feature, int):
            ...
            block_idx = InceptionV3.BLOCK_INDEX_BY_DIM[feature]
            ...
            if hasattr(self.inception, "num_features"):
                ...
                if self.normalize:
                    ...
                    dummy_image = torch.randint(
                        0, 255, (1, *input_img_size), dtype=torch.uint8
                    )
        ...
        # use cache
        self.use_cache = fid_statistics_file is not None
        if self.use_cache:
            data = np.load(fid_statistics_file)
            self.mu = torch.tensor(data["mu"], dtype=torch.double)
            self.sigma = torch.tensor(data["sigma"], dtype=torch.double)
    ...
    def update(self, imgs: Tensor, real: bool) -> None:
        ...
        """
        # print(self.normalize, imgs.dtype)
        imgs = (
            (imgs * 255).byte()
        ...
        # print(imgs.shape, imgs.dtype, torch.min(imgs), torch.max(imgs))
        features = self.inception(imgs)[0].squeeze(3).squeeze(2)
        # print("features", torch.mean(features))
        # print(features.dtype, features.shape, torch.min(imgs), torch.max(imgs))
    ...
    def compute(self) -> Tensor:
        ...
        if self.use_cache:
            mean_real = self.mu.to(torch.cuda.current_device()).unsqueeze(0)
            cov_real = self.sigma.to(torch.cuda.current_device())
        else:
            mean_real = (
