# python openai_evaluator.py /mnt/iem-nas/home/qinzhen/qinzhen/experiment/dit/reference/VIRTUAL_imagenet256_labeled.npz /mnt/iem-nas/home/qinzhen/qinzhen/experiment_generation/fsq/script/sample/val/data.npz
#
# IS: 232.12393188476562
# FID: 1.6769560401259014
# sFID: 3.669224528927998
# Precision: 0.75264
# Recall: 0.667
...
def OpenaiEvaluator(ref_batch, sample_batch):
    ...
    logging_info("warming up TensorFlow...")
    # This will cause TF to print a bunch of verbose stuff now rather
    # than after the next logging_info(), to help prevent confusion.
    evaluator.warmup()

    logging_info("computing reference batch activations...")
    ref_acts = evaluator.read_activations(ref_batch)
    logging_info("computing/reading reference batch statistics...")
    ref_stats, ref_stats_spatial = evaluator.read_statistics(ref_batch, ref_acts)

    logging_info("computing sample batch activations...")
    sample_acts = evaluator.read_activations(sample_batch)
    logging_info("computing/reading sample batch statistics...")
    sample_stats, sample_stats_spatial = evaluator.read_statistics(
        sample_batch, sample_acts
    )

    logging_info("Computing evaluations...")
    IS = evaluator.compute_inception_score(sample_acts[0])
    FID = sample_stats.frechet_distance(ref_stats)
    sFID = sample_stats_spatial.frechet_distance(ref_stats_spatial)
    logging_info("Inception Score:", IS)
    logging_info("FID:", FID)
    logging_info("sFID:", sFID)
    prec, recall = evaluator.compute_prec_recall(ref_acts[0], sample_acts[0])
    logging_info("Precision:", prec)
    logging_info("Recall:", recall)
...
def _download_inception_model():
    if os.path.exists(INCEPTION_V3_PATH):
        return
    logging_info("downloading InceptionV3 model...")
