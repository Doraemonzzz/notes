...
def _fwd_diag_kernel(
    ...
    Out,
    S,
    b: tl.constexpr, h: tl.constexpr, n: tl.constexpr, d: tl.constexpr, e: tl.constexpr,
    ...
    for j in range(i + 1):
        ...
        k_trans = tl.load(K_trans_block_ptr, mask=kv_index[None, :] < n, other=0.0).to(tl.float32)
...
def _fwd_kv_parallel(
    ...
    b: tl.constexpr, h: tl.constexpr, n: tl.constexpr, d: tl.constexpr, e: tl.constexpr,
    BLOCK: tl.constexpr, NUM_BLOCK: tl.constexpr, 
    D_FBLOCK: tl.constexpr, E_FBLOCK: tl.constexpr, NUM_FBLOCK: tl.constexpr,
    CBLOCK: tl.constexpr, NUM_CBLOCK: tl.constexpr, 
):
    start_m = 0
    ...
    # (CBLOCK, FBLOCK)
    K_trans_block_ptr = K + k_offset + k_block_offset + d_offset + \
                        tl.arange(0, CBLOCK)[None, :] * d + \
                        tl.arange(0, D_FBLOCK)[:, None]
    V_block_ptr = V + v_offset + v_block_offset + e_offset + \
                  tl.arange(0, CBLOCK)[:, None] * e + \
                  tl.arange(0, E_FBLOCK)[None, :]
    KV_block_ptr = KV + kv_offset + kv_block_offset + d_offset * e + e_offset + \
...
def _fwd_kv_reduce(
    ...
    # (CBLOCK, FBLOCK)
    KV_block_ptr = KV + kv_offset + d_offset * e + e_offset + \
              tl.arange(0, D_FBLOCK)[:, None] * e + \
              tl.arange(0, E_FBLOCK)[None, :]
...
def _fwd_none_diag_kernel(
    ...
    Q_block_ptr = Q + q_offset + tl.arange(0, BLOCK)[:, None] * d + tl.arange(0, d)[None, :]
    O_block_ptr = Out + o_offset + tl.arange(0, BLOCK)[:, None] * e + tl.arange(0, E_FBLOCK)[None, :]
...
def _bwd_diag_kernel(
    ...
    # dq
    DO_block_ptr = DO + o_offset + o_block_offset + o_cblock_offset + tl.arange(0, CBLOCK)[:, None] * e + tl.arange(0, e)[None, :]
    DQ_block_ptr = DQ + qk_offset + qk_block_offset + qk_cblock_offset + tl.arange(0, CBLOCK)[:, None] * d + tl.arange(0, d)[None, :]
    K_block_ptr = K + qk_offset + qk_block_offset + tl.arange(0, CBLOCK)[:, None] * d + tl.arange(0, d)[None, :]
    V_trans_block_ptr = V + v_offset + v_block_offset + tl.arange(0, CBLOCK)[None, :] * e + tl.arange(0, e)[:, None]
    ...
    # dk
    V_trans_block_ptr = V + v_offset + v_block_offset + v_cblock_offset + tl.arange(0, CBLOCK)[None, :] * e + tl.arange(0, e)[:, None]
    DO_block_ptr = DO + o_offset + o_block_offset + o_cblock_offset + tl.arange(0, CBLOCK)[:, None] * e + tl.arange(0, e)[None, :]
    Q_trans_block_ptr = Q + qk_offset + qk_block_offset + qk_cblock_offset + tl.arange(0, CBLOCK)[None, :] * d + tl.arange(0, d)[:, None]
    DK_trans_block_ptr = DK + qk_offset + qk_block_offset + qk_cblock_offset + tl.arange(0, CBLOCK)[None, :] * d + tl.arange(0, d)[:, None]
    ...
    # add
    K_block_ptr = K + qk_offset + qk_block_offset + qk_cblock_offset + tl.arange(0, CBLOCK)[:, None] * d + tl.arange(0, d)[None, :]
    DV_block_ptr = DV + v_offset + v_block_offset + v_cblock_offset + tl.arange(0, CBLOCK)[:, None] * e + tl.arange(0, e)[None, :]
...
def _bwd_dkv_parallel(
    ...
    # (CBLOCK, FBLOCK)
    DKV_block_ptr = DKV + kv_offset + kv_block_offset + d_offset * e + e_offset + \
                   tl.arange(0, D_FBLOCK)[:, None] * e + \
                   tl.arange(0, E_FBLOCK)[None, :]
                   
    Q_trans_block_ptr = Q + qk_offset + qk_block_offset + d_offset + tl.arange(0, CBLOCK)[None, :] * d + tl.arange(0, D_FBLOCK)[:, None]
    DO_block_ptr = DO + o_offset + o_block_offset + e_offset + tl.arange(0, CBLOCK)[:, None] * e + tl.arange(0, E_FBLOCK)[None, :]
...
def _bwd_dkv_reduce(
    ...
    DKV_block_ptr = DKV + kv_offset + d_offset * e + e_offset + \
              NUM_BLOCK * d * e + \
              tl.arange(0, D_FBLOCK)[:, None] * e + \
              tl.arange(0, E_FBLOCK)[None, :]
...
def _bwd_none_diag_kernel(
    ...
    KV_trans_block_ptr = KV + kv_trans_offset + d_offset * e + tl.arange(0, D_FBLOCK)[None, :] * e + tl.arange(0, e)[:, None]
    DQ_block_ptr = DQ + qk_offset + d_offset + tl.arange(0, CBLOCK)[:, None] * d + tl.arange(0, D_FBLOCK)[None, :]
    ...
    # dk
    DK_trans_block_ptr = DK + qk_offset + d_offset + tl.arange(0, CBLOCK)[None, :] * d + tl.arange(0, D_FBLOCK)[:, None]
    DKV_block_ptr = DKV + kv_offset + d_offset * e + tl.arange(0, D_FBLOCK)[:, None] * e + tl.arange(0, e)[None, :]
    V_trans_block_ptr = V + v_offset + tl.arange(0, CBLOCK)[None, :] * e + tl.arange(0, e)[:, None]
    
    v_trans = tl.load(V_trans_block_ptr).to(tl.float32)
    dkv = tl.load(DKV_block_ptr).to(tl.float32)
    k_decay_trans = tl.exp(-s.to(tl.float32) * (BLOCK - (off_c * CBLOCK + c_array[None, :])))
    ...
    # dv
    DKV_block_ptr_ = DKV + kv_offset + e_offset + tl.arange(0, d)[:, None] * e + tl.arange(0, E_FBLOCK)[None, :]
    K_block_ptr = K + qk_offset + tl.arange(0, CBLOCK)[:, None] * d + tl.arange(0, d)[None, :]
    DV_block_ptr = DV + v_offset + e_offset + tl.arange(0, CBLOCK)[:, None] * e + tl.arange(0, E_FBLOCK)[None, :]
...
class _attention(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, s):
        ...
        _fwd_diag_kernel[grid](
            ...
            CBLOCK=CBLOCK, NUM_CBLOCK=NUM_CBLOCK
        )
...
lightning_attention2 = _attention.apply

def lightning_attention2_parallel(q, k, v, ed):
    d = q.shape[-1]
    e = v.shape[-1]
    # arr = f(d)
    if d >= 128:
    ...
    for i in range(n - 1):
        ...
        k1 = k[..., s:e]  # .contiguous()
        # print(output.shape)
        o = lightning_attention2(q1, k1, v, ed)
        output = output + o

    return output

# lightning_attention2_parallel = _attention.apply